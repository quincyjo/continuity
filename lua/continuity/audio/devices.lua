...
function devices.new()
	local AudioDevice = class.new("AudioDevice"):with(Subscribable):with(Controllable):with(Removable)({
		methods = {
	...
	local function bind(api_sub)
		...
		AudioDevice._methods.adjust_perc = function(self, delta)
		...
		AudioDevice._methods.set_perc = function(self, value)
		...
		AudioDevice._methods.toggle_mute = function(self)
			api_sub.toggle(self.id, function(level, muted)
				update_level_muted(self, level, muted)
			end)
		end

		AudioDevice._methods.set_default = function(self)
			api_sub.set_default(self.id, function()
				self:control_event(self.state)
			end)
		end

		AudioDevice._methods.set_port = function(self, port)
