...
local function mock_source(initial_state)
	local cbs = {}
	local src
	src = {
		state = initial_state,
...
describe("History", function()
	...
	describe("iter", function()
		...
		it("Scenario 2: 15 pushes on capacity=10 yields entries 6..15 in order", function()
			local src = mock_source()
			local h = History(src, 10)
			for i = 1, 15 do
				src.push(i)
	...
	describe("stop / start / reset", function()
		...
		it("Scenario 4: stop -> reset -> start lifecycle", function()
			local src = mock_source()
			local h = History(src, 10)
			src.push(1)
			src.push(2)
